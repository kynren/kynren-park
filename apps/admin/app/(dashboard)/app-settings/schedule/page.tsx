...
export default function ScheduleAdmin() {
  ...
  function openNew() { setForm({ ...EMPTY, attractionId: attractions[0]?.id ?? '' }); setError(''); }
  ...
  async function save() {
    ...
    try {
      if (form.id) await api(`/admin/sessions/${form.id}`, { method: 'PATCH', body: JSON.stringify({ start: form.start, end: form.end, status: form.status }) });
      else await api('/admin/sessions', { method: 'POST', body: JSON.stringify({ attractionId: form.attractionId, date, start: form.start, end: form.end, status: form.status }) });
      setForm(null); load(date);
    } catch { setError('Save failed — check the times.'); }
  }
      ...
      {form && (
        <div className="modal-back" onClick={(e) => e.target === e.currentTarget && setForm(null)}>
          <div className="modal">
            ...
            <p className="hint">Date: {date}. Times are park time (UTC).</p>
