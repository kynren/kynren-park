...
interface Attraction { id: string; name: string; category: string }
interface Session {
...
export default function ShowsAdmin() {
  ...
  const load = useCallback((d: string) => {
    api<Session[]>(`/schedule?date=${d}`).then(setSessions).catch((e) => setError(friendlyError(e, 'Could not load shows.')));
  }, []);
  ...
  async function save() {
    ...
    } catch (e) { setError(friendlyError(e, 'Save failed — check the times and range.')); }
  }

  async function remove(s: Session) {
    if (!(await confirmDelete(`Delete the ${fmt(s.startTime)} ${s.attraction.name} show?`))) return;
  ...
  return (
    <div>
      <div className="page-actions" style={{ marginTop: 0 }}>
        <div>
          <h1>Shows</h1>
          <p className="subtitle" style={{ margin: 0 }}>The programme of show times guests see in the mobile timetable.</p>
        </div>
        <div style={{ display: 'flex', gap: 10 }}>
          <input type="date" value={date} onChange={(e) => setDate(e.target.value)} />
          <button className="primary" onClick={openNew}>+ Add show time</button>
        </div>
      </div>
      ...
      <div className="tt">
        ...
        {byAttraction.length === 0 && <div className="empty-chart" style={{ padding: 8 }}>No shows scheduled for {date}.</div>}
        {byAttraction.map((a) => (
      ...
      {form && (
        <div className="modal-back" onClick={(e) => e.target === e.currentTarget && setForm(null)}>
          <div className="modal">
            <h2>{form.id ? 'Edit show time' : 'Add show time'}</h2>
            ...
            {!form.id && (
              <div style={{ marginTop: 14, borderTop: '1px solid var(--line)', paddingTop: 14 }}>
                <label className="checkline" style={{ fontWeight: 600 }}>
                  <input type="checkbox" checked={repeat} onChange={(e) => setRepeat(e.target.checked)} /> Repeat this show
                </label>
                {repeat && (
                  <div style={{ marginTop: 12, display: 'grid', gap: 12 }}>
                    ...
                    <p className="hint" style={{ margin: 0 }}>Creates one show per selected weekday from {date} to {until || '…'}.</p>
                  </div>
                )}
              </div>
            )}
